  ...
  <div class="min-h-[80vh]">
    <div class="max-w-lg mb-10 border border-gray-700 rounded-md p-5 dark:bg-gray-800">
      <div class="w-full">
        ...
        <div
          :class="{
            'bg-red-500 rounded-md text-white dark:text-gray-900': store.temp == 'Error',
          }"
          class="flex justify-end px-4 py-2 dark:text-white border-b-4 mb-6 border-b-blue-900 border-r-4 border-r-blue-900 dark:border-r dark:border-b dark:border-b-gray-400 dark:border-r-gray-300"
        >
          {{ store.temp }}
        </div>
        <input
          disabled
          class="w-full bg-gray-200 rounded-md px-4 py-2 text-right dark:bg-black dark:text-white"
          type="text"
          v-model="store.input"
        />
      </div>
      <div class="w-full grid grid-flow-row grid-cols-4 mt-5 gap-5">
        <div
          class="w-full grid grid-flow-row grid-cols-3 gap-5 col-span-3 dark:text-white"
        >
          <button
            @click="clear()"
            type="button"
          ...
          <button
            ...
            type="button"
            class="border border-gray-700 p-4 bg-gray-100 rounded-md dark:bg-black active:bg-green-200"
            v-for="number in 9"
            :key="number"
          >
            {{ number }}
        ...
        <div class="w-full grid grid-flow-row gap-5">
          <button
            ...
            type="button"
            class="border border-gray-700 p-4 bg-gray-100 rounded-md dark:bg-black active:bg-green-200"
            v-for="(type, operator) in operators"
            :key="operator"
          >
            {{ type }}
          </button>
